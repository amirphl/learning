...
public:
  ...
  int countRestrictedPaths(int n, vector<vector<int>> &edges) {
    ...
    dfs(adj, dist, visited, path, 1, -1, 0, count);

    return count;
  }

  void dfs(const vector<vector<pair<int, int>>> &adj, const vector<int> &dist,
           vector<bool> &visited, vector<int> &path, int z, int prev_z,
           int level, int &count) {
    int n = visited.size() - 1;
    visited[z] = 1;
    // path[level] = z;

    if (level > 0 && dist[prev_z] <= dist[z]) {
      visited[z] = 0; // Spent one hour to debug. I forgot the line!
      return;
    }
    ...
    for (auto &p : adj[z]) {
      ...
      if (next_z == n) {
        ...
        // }
        // cout << next_z << endl;
      } else if (!visited[next_z]) {
        dfs(adj, dist, visited, path, next_z, z, level + 1, count);
      }
    }
