...
// time: O(n), memory: O(h)
Node* cnstrct(int pre[], int& i, int low, int high, int n) {
    if (i >= n || pre[i] < low || pre[i] > high) {
        return 0;
    }
    Node* root = newNode(pre[i]);
    i++;
    root -> left = cnstrct(pre, i, low, root -> data, n);
    root -> right = cnstrct(pre, i, root -> data, high, n);
    return root;
}
...
{
    int i = 0;
    return cnstrct(pre, i, INT_MIN, INT_MAX, n);
}
