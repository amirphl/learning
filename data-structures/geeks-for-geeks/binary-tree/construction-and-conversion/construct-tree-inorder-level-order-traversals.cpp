...
Node* buildTreeRec(int in[], int lvl[], int low, int high, int left, int right, std::map<int, int> &m) {
    ...
    // spend 1 hour to debug why there is a seg fault. Interesting part is you can prevent seg fault by std::cout << std::endl;
    ...
    // std::set<int> s(in + low, in + root_index);
    std::unordered_set<int> s(in + low, in + root_index);
