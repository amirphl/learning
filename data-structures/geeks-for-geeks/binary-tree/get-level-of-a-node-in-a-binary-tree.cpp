...
public:
    ...
    {
        return getLevelRec(node, target, 1);
    }

    // time: O(n), memory: O(h)
    int getLevelRec(struct Node* node, int target, int lvl) {
        if (!node) {
            return 0;
        }
        if (node -> data == target) {
        ...
        int left = getLevelRec(node -> left, target, lvl + 1);
        if (left != 0) {
            return left;
        }
        int right = getLevelRec(node -> right, target, lvl + 1);
        if (right != 0) {
            return right;
        }
        return 0;
